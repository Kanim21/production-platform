...
module "vpc" {
  ...
  # Single NAT in dev — cost savings; not HA but acceptable for dev
...
module "eks" {
  ...
  # Allow access from anywhere in dev (engineers' home IPs vary)
...
module "rds" {
  ...
  reader_count   = 0 # No reader in dev — cost savings
...
module "monitoring" {
  ...
  alb_arn_suffix         = "" # Populated after ALB is created by LB controller
